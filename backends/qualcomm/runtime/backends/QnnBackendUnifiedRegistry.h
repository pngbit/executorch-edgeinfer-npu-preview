...
// Returns a canonical identity for every option that can affect shared QNN
// backend/device compatibility. Returning nullopt means the options are
// incomplete and the caller must not cache or share a bundle.
...
class QnnBackendUnifiedRegistry {
 ...
 private:
  ...
  // Bundles are shared only for byte-identical canonical compatibility
  // identities. This is deliberately conservative: a missed sharing
  // opportunity costs memory, while sharing incompatible QNN resources can
  // corrupt execution.
  std::unordered_map<std::string, std::weak_ptr<QnnBackendBundle>>
