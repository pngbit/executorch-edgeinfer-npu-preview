...
Result<DelegateHandle*> QnnExecuTorchBackend::init(
  ...
  const bool has_context_signature = status == Error::Ok;
  if (status == Error::Ok) {
  ...
  ET_CHECK_OR_RETURN_ERROR(
      qnn_executorch_options != nullptr,
      InvalidArgument,
      "Missing QNN compile spec");

  std::optional<std::string> delegate_cache_identity;
  if (has_context_signature) {
    delegate_cache_identity =
        BuildQnnDelegateCacheIdentity(signature, qnn_executorch_options);
  }
  ...
  if (delegate_cache_identity.has_value()) {
    if (DelegateHandle* cached =
            delegate_registry_.acquire(*delegate_cache_identity)) {
      QNN_EXECUTORCH_LOG_INFO(
  ...
  auto manager_guard = std::unique_ptr<QnnManager, void (*)(QnnManager*)>(
      qnn_manager, [](QnnManager* manager) { manager->~QnnManager(); });
  ...
  DelegateHandle* delegate = qnn_manager;
  if (delegate_cache_identity.has_value()) {
    delegate = delegate_registry_.cache_or_acquire(
        *delegate_cache_identity, qnn_manager);
    ET_CHECK_OR_RETURN_ERROR(
        delegate != nullptr,
        Internal,
        "QNN manager candidate is already registered");
    if (delegate != qnn_manager) {
      // Another method may have initialized the same shared context while this
      // method was preparing. Keep the cached instance and release this one.
    } else {
      manager_guard.release();
    }
  } else if (!delegate_registry_.track_unique(qnn_manager)) {
...
Error QnnExecuTorchBackend::execute(
  ...
  // The lease closes the contains()/destroy() race and serializes mutation of
  // the shared QnnManager tensor wrappers across methods. destroy() removes the
  // handle from the registry first, then waits for this lease to drain.
  auto execution = delegate_registry_.acquire_execution(handle);
  ET_CHECK_OR_RETURN_ERROR(
      execution, Internal, "DelegateHandle has been deleted");
  QnnManager* qnn_manager = static_cast<QnnManager*>(execution.get());
  ...
  std::vector<SharedBuffer::AllocationLease> allocation_leases;
  allocation_leases.reserve(args.size());
  auto pin_custom_allocation = [&](void* data) {
    auto lease = SharedBuffer::GetSharedBufferManager().AcquireAllocation(data);
    if (lease) {
      allocation_leases.emplace_back(std::move(lease));
    }
  };
  ...
  for (const auto& input_tensor : input_tensors) {
    if (input_tensor->GetName().find("mutbuf_") == std::string::npos) {
      void* data_ptr = args[args_index]->toTensor().mutable_data_ptr();
      pin_custom_allocation(data_ptr);
      if (qnn_manager->RegisterMem(data_ptr, input_tensor) != Error::Ok) {
  ...
  for (const auto& output_tensor : output_tensors) {
    ...
    if (output_tensor->GetName().rfind("output_", 0) == 0 &&
      ...
      pin_custom_allocation(mutable_data_ptr);
...
void QnnExecuTorchBackend::destroy(DelegateHandle* handle) const {
  delegate_registry_.release_and_destroy(handle, [](DelegateHandle* owned) {
    static_cast<QnnManager*>(owned)->~QnnManager();
  });
}
