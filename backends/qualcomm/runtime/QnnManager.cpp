...
Error QnnManager::RegisterMem(
  ...
  // Explicit custom-memory allocations are safe to register even when an
  // older PTE did not set the global shared_buffer compile option. Ordinary
  // buffers retain the existing opt-in behavior.
  if (!options_->shared_buffer() && custom_mem_base == nullptr) {
...
Error QnnManager::RegisterIonMem(
    ...
    const std::shared_ptr<TensorWrapper>& tensor_wrapper) {
  SharedBuffer& shared_buffer_manager = SharedBuffer::GetSharedBufferManager();
  auto allocation_lease = shared_buffer_manager.AcquireAllocation(data_ptr);
  if (!allocation_lease) {
  ...
  int32_t mem_fd = shared_buffer_manager.MemToFd(data_ptr, allocation_lease);
  if (mem_fd == -1) {
    QNN_EXECUTORCH_LOG_WARN(
...
Error QnnManager::RegisterCustomMem(
    ...
    const std::shared_ptr<TensorWrapper>& tensor_wrapper) {
  SharedBuffer& shared_buffer_manager = SharedBuffer::GetSharedBufferManager();
  auto allocation_lease =
      shared_buffer_manager.AcquireAllocation(custom_mem_base);
  if (!allocation_lease) {
    return Error::Internal;
  }
  ...
  auto aligned_tensor_offset = GetAddressOffset(custom_mem_base, data_ptr);
  if (!aligned_tensor_offset.has_value()) {
    QNN_EXECUTORCH_LOG_WARN(
        "Tensor name %s is outside its custom shared-memory base.",
        tensor_wrapper->GetName().c_str());
    return Error::Internal;
  }
  ...
  size_t total_custom_mem_size = 0;
  if (!shared_buffer_manager.GetCustomMemDescriptorRange(
          custom_mem_base,
          info.pos,
          info.tensor_bytes,
          ...
          &tensor_offset)) {
    QNN_EXECUTORCH_LOG_WARN(
        "Tensor name %s has an invalid custom shared-memory range.",
        tensor_wrapper->GetName().c_str());
    return Error::Internal;
  }
  ...
  int32_t mem_fd =
      shared_buffer_manager.MemToFd(custom_mem_base, allocation_lease);
...
void QnnExecuTorchFreeCustomMem(void* buffer_ptr) {
  auto& shared_buffer =
      executorch::backends::qnn::SharedBuffer::GetSharedBufferManager();
  auto bytes = shared_buffer.BeginFree(buffer_ptr);
  if (!bytes.has_value()) {
    return;
  }
  if (executorch::backends::qnn::QnnMemManager::DeregisterAllMemRange(
          buffer_ptr, *bytes) != executorch::runtime::Error::Ok) {
    shared_buffer.CancelFree(buffer_ptr);
    executorch::backends::qnn::Log(
        qnn_delegate::QnnExecuTorchLogLevel::kLogLevelError,
        "Failed to de-register QNN handles for custom memory %p; preserving "
        "the allocation to avoid stale handles.",
        buffer_ptr);
    return;
  }
  shared_buffer.CommitFree(buffer_ptr);
}
