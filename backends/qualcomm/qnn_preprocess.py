...
class QnnBackend(BackendDetails):
            ...
            if node.op == "call_function":
                ...
                if node.target.__name__ in node_visitors:
                    ...
                    try:
                        ...
                        context_loader_target = getattr(
                            torch.ops, OpContextLoader.namespace
                        )
                        for attribute in op.__name__.split("."):
                            context_loader_target = getattr(
                                context_loader_target, attribute
                            )
    ...
    def preprocess_multimethod(  # noqa: C901
        ...
        # Online-prepare serializes DLC data rather than a context binary. The
        # QNN IR path currently emits only the first graph when Compile() is
        # given multiple graph names, so a shared DLC would make every method
        # execute that first graph. Compile one DLC per method instead. Offline
        # context binaries keep the existing shared multi-graph path below.
        if option.online_prepare:
            for i in range(num_sub_graphs):
                for j, (graph_name, programs) in enumerate(edge_programs.items()):
                    ...
                    qnn_manager.InitContext([graph_name])
                    ...
                    if isinstance(py_op_wrappers, bytes):
                        processed_bytes = py_op_wrappers
                    else:
                        qnn_context_binary = qnn_manager.Compile(
                            [graph_name],
                            [
                        ...
                        ), "Failed to generate Qnn DLC."
                        processed_bytes = bytes(qnn_context_binary)
                    qnn_manager.DestroyContext()
                    all_processed_results[graph_name].append(
                        PreprocessResult(
