...
class KVManager {
 public:
  ...
  /** Validate a contiguous Float/Half host-composed K/V state. */
  executorch::runtime::Error validate_layer_cache(
      int64_t layer,
      const executorch::aten::Tensor& key,
      const executorch::aten::Tensor& value,
      int32_t valid_length) const;

  /** Replace one native layer cache, converting only when dtypes differ. */
  executorch::runtime::Error replace_layer_cache(
      int64_t layer,
      const executorch::aten::Tensor& key,
      const executorch::aten::Tensor& value,
      int32_t valid_length);

  /** Backward-compatible validation for callers that require FP32 input. */
  executorch::runtime::Error validate_fp32_layer_cache(
      int64_t layer,
      const executorch::aten::Tensor& key,
      const executorch::aten::Tensor& value,
      int32_t valid_length) const;
  ...
  /**
   * Replace one layer's native cache with host-composed FP32 K/V state.
   *
   * The source layouts are key `[1, H, D, L]` and value `[1, H, L, D]`.
   * Native FP32 and IEEE FP16 caches are supported. Quantized UInt8/UInt16
   * caches require their exported scale/zero-point contract and are rejected.
   */
  executorch::runtime::Error replace_fp32_layer_cache(
      int64_t layer,
      const executorch::aten::Tensor& key,
      const executorch::aten::Tensor& value,
      int32_t valid_length);

  inline executorch::aten::ScalarType cache_scalar_type() const {
    return kv_cache_dtype_;
  }

  inline int32_t current_ar_len() const {
    return cur_ar_len_;
  }
