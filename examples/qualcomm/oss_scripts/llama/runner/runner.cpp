...
Runner::Runner(
    ...
    : module_(std::move(module)),
...
#if defined(EXECUTORCH_BUILD_EDGEINFER)
      ,
      edgeinfer_composable_module_(std::move(edgeinfer_composable_module)),
      ...
      edgeinfer_prefill_options_(std::move(edgeinfer_prefill_options))
#endif
      ,
      ngram_(ngram),
      window_(window),
...
Error Runner::load() {
  ...
  const PromptProcessor::Metadata prompt_metadata{
      context_len_,
      num_heads,
      num_layers,
      ...
      sliding_window,
      cache_mode_};
  auto prompt_method_meta = std::make_unique<MethodMeta>(
      std::move(module_->method_meta(prompt_processor_method_name).get()));
#if defined(EXECUTORCH_BUILD_EDGEINFER)
  if (edgeinfer::should_use_edgeinfer_prompt_processor(
          edgeinfer_composable_module_.get(),
          eval_mode_ == EvalMode::kLookaheadDecoding)) {
    std::vector<executorch::extension::Module*> edgeinfer_layer_modules;
    edgeinfer_layer_modules.reserve(edgeinfer_composable_layer_modules_.size());
    for (const auto& layer_module : edgeinfer_composable_layer_modules_) {
      edgeinfer_layer_modules.emplace_back(layer_module.get());
    }
    std::vector<executorch::extension::Module*> edgeinfer_decode_layer_modules;
    edgeinfer_decode_layer_modules.reserve(
        edgeinfer_decode_composable_layer_modules_.size());
    for (const auto& layer_module :
         edgeinfer_decode_composable_layer_modules_) {
      edgeinfer_decode_layer_modules.emplace_back(layer_module.get());
    }
    auto edgeinfer_processor = std::make_unique<EdgeInferPromptProcessor>(
        decoder_runner_.get(),
        kv_manager_.get(),
        prompt_processor_method_name,
        prompt_metadata,
        std::move(prompt_method_meta),
        edgeinfer_composable_module_.get(),
        std::move(edgeinfer_layer_modules),
        edgeinfer_decode_composable_module_.get(),
        std::move(edgeinfer_decode_layer_modules),
        token_generator_ar_len,
        edgeinfer_prefill_options_);
    edgeinfer_prompt_processor_ = edgeinfer_processor.get();
    prompt_processor_ = std::move(edgeinfer_processor);
    ET_LOG(
        Info,
        "EdgeInfer split-PTE Prefill is enabled as an opt-in path; "
        "the original decoder PTE remains the fallback path.");
  } else {
    if (edgeinfer_composable_module_ != nullptr &&
        eval_mode_ == EvalMode::kLookaheadDecoding) {
      ET_LOG(
          Info,
          "EdgeInfer split execution does not support Lookahead Decoding; "
          "using the original PromptProcessor.");
    }
...
Error Runner::generate_from_prompt_or_file(
  ...
  if (cur_pos_ >= seq_len - 1) {
    num_generated_tokens = 0;
  }
#if defined(EXECUTORCH_BUILD_EDGEINFER)
  else if (
      edgeinfer_prompt_processor_ != nullptr &&
      edgeinfer_prompt_processor_->has_edgeinfer_state() &&
      attention_sink_rope_runner_ == nullptr) {
    const Error decode_prepare =
        edgeinfer_prompt_processor_->prepare_decode(seq_len - 1);
    if (decode_prepare == Error::Ok) {
      ET_LOG(
          Info,
          "EdgeInfer split Decode is enabled with persistent block-native KV; "
          "the native kv_forward path remains the pre-execution fallback.");
      int64_t pos = cur_pos_;
      Error decode_error = Error::Ok;
      while (pos < seq_len - 1) {
        const uint64_t previous_token = cur_token;
        const double decode_token_start_ms = time_in_ms();
        stats_.on_model_execution_begin();
        auto decode_result = edgeinfer_prompt_processor_->decode_token(
            cur_token, pos, dump_logits);
        stats_.on_model_execution_end();
        if (!decode_result.ok()) {
          decode_error = decode_result.error();
          break;
        }
        cur_token = decode_result.get();
        ++pos;
        ET_LOG(
            Info,
            "EdgeInfer Decode token position=%lld latency=%.3f ms.",
            static_cast<long long>(pos - 1),
            time_in_ms() - decode_token_start_ms);

        if (token_callback) {
          ET_ASSIGN_OR_RETURN_TOKENIZER(
              decoded_token__, tokenizer_->decode(previous_token, cur_token));
          token_callback(decoded_token__);
        }
        if (edgeinfer_eos_ids_.count(cur_token) > 0) {
          ET_LOG(Info, "Reached the end of generation in EdgeInfer Decode.");
          break;
        }
      }
      const int64_t successful_edgeinfer_tokens = pos - cur_pos_;
      if (decode_error == Error::Ok) {
        num_generated_tokens = successful_edgeinfer_tokens;
      } else {
        ET_LOG(
            Info,
            "EdgeInfer Decode failed at position=%lld after %lld committed "
            "tokens (0x%x); bridging the transactionally restored K/V state "
            "to native kv_forward.",
            static_cast<long long>(pos),
            static_cast<long long>(successful_edgeinfer_tokens),
            static_cast<unsigned int>(decode_error));
        auto recovered_generate =
            llm::resume_native_decode_after_edgeinfer_failure(
                successful_edgeinfer_tokens,
                cur_token,
                pos,
                [&]() {
                  return edgeinfer_prompt_processor_->bridge_to_native_decode();
                },
        ...
        if (!recovered_generate.ok()) {
          ET_LOG(
              Error,
              "Native Decode recovery failed at position=%lld (0x%x); the "
              "original EdgeInfer error was 0x%x.",
              static_cast<long long>(pos),
              static_cast<unsigned int>(recovered_generate.error()),
              static_cast<unsigned int>(decode_error));
          return recovered_generate.error();
        }
        num_generated_tokens = recovered_generate.get();
      }
    } else {
      const Error bridge_error =
          edgeinfer_prompt_processor_->bridge_to_native_decode();
      ET_CHECK_OK_OR_RETURN_ERROR(bridge_error);
      ET_LOG(
          Info,
          "EdgeInfer Decode preparation failed (0x%x); using native kv_forward "
          "after materializing the block-native K/V state.",
          static_cast<unsigned int>(decode_prepare));
      auto native_generate = token_generator_->generate(
          prompt_tokens,
          cur_pos_,
          seq_len,
          token_callback,
          dump_logits,
          attention_sink_rope_runner_.get());
      ET_CHECK_OK_OR_RETURN_ERROR(native_generate.error());
      num_generated_tokens = native_generate.get();
    }
  }
#endif
  else {
