...
executorch::runtime::Error KVManager::validate_layer_cache(
    int64_t layer,
    const executorch::aten::Tensor& key,
    const executorch::aten::Tensor& value,
    int32_t valid_length) const {
  using executorch::aten::ScalarType;
  using executorch::runtime::Error;
  if ((kv_cache_dtype_ != ScalarType::Float &&
       kv_cache_dtype_ != ScalarType::Half) ||
      layer < 0 || layer >= metadata_.num_layers || valid_length < 0 ||
      valid_length > metadata_.max_cache_len ||
      !executorch::runtime::tensor_is_contiguous(key) ||
      !executorch::runtime::tensor_is_contiguous(value) ||
      (key.scalar_type() != ScalarType::Float &&
       key.scalar_type() != ScalarType::Half) ||
      value.scalar_type() != key.scalar_type() || key.dim() != 4 ||
      value.dim() != 4 || key.size(0) != 1 || value.size(0) != 1 ||
      key.size(1) != metadata_.num_heads ||
      value.size(1) != metadata_.num_heads ||
      key.size(2) != metadata_.head_dim ||
      value.size(3) != metadata_.head_dim || key.size(3) != valid_length ||
      value.size(2) != valid_length) {
    return kv_cache_dtype_ == ScalarType::Float ||
            kv_cache_dtype_ == ScalarType::Half
...
executorch::runtime::Error KVManager::validate_fp32_layer_cache(
    int64_t layer,
    const executorch::aten::Tensor& key,
    const executorch::aten::Tensor& value,
    int32_t valid_length) const {
  if (key.scalar_type() != executorch::aten::ScalarType::Float ||
      value.scalar_type() != executorch::aten::ScalarType::Float) {
    return executorch::runtime::Error::InvalidArgument;
  }
  return validate_layer_cache(layer, key, value, valid_length);
}

executorch::runtime::Error KVManager::replace_layer_cache(
    int64_t layer,
    const executorch::aten::Tensor& key,
    const executorch::aten::Tensor& value,
  ...
  const Error validation =
      validate_layer_cache(layer, key, value, valid_length);
  if (validation != Error::Ok) {
    return validation;
  }

  const size_t cache_length = static_cast<size_t>(metadata_.max_cache_len);
  const size_t heads = static_cast<size_t>(metadata_.num_heads);
  const size_t head_dim = static_cast<size_t>(metadata_.head_dim);
  auto& native_key = k_cache_[static_cast<size_t>(layer)];
  auto& native_value = v_cache_[static_cast<size_t>(layer)];
  const size_t valid = static_cast<size_t>(valid_length);
  using Half = executorch::aten::Half;

  if (key.scalar_type() == ScalarType::Float &&
      kv_cache_dtype_ == ScalarType::Float) {
    edgeinfer::copy_layer_cache_data<float, float>(
        key.const_data_ptr<float>(),
        value.const_data_ptr<float>(),
        reinterpret_cast<float*>(native_key.buffer),
        reinterpret_cast<float*>(native_value.buffer),
        heads,
        head_dim,
        valid,
        cache_length);
      ...
      key.scalar_type() == ScalarType::Float &&
      kv_cache_dtype_ == ScalarType::Half) {
    edgeinfer::copy_layer_cache_data<float, Half>(
        key.const_data_ptr<float>(),
        value.const_data_ptr<float>(),
        reinterpret_cast<Half*>(native_key.buffer),
        reinterpret_cast<Half*>(native_value.buffer),
        heads,
        head_dim,
        valid,
        cache_length);
      ...
      key.scalar_type() == ScalarType::Half &&
      kv_cache_dtype_ == ScalarType::Float) {
    edgeinfer::copy_layer_cache_data<Half, float>(
        key.const_data_ptr<Half>(),
        value.const_data_ptr<Half>(),
        reinterpret_cast<float*>(native_key.buffer),
        reinterpret_cast<float*>(native_value.buffer),
        heads,
        head_dim,
    ...
    edgeinfer::copy_layer_cache_data<Half, Half>(
        key.const_data_ptr<Half>(),
        value.const_data_ptr<Half>(),
        reinterpret_cast<Half*>(native_key.buffer),
        reinterpret_cast<Half*>(native_value.buffer),
...
executorch::runtime::Error KVManager::replace_fp32_layer_cache(
    int64_t layer,
    const executorch::aten::Tensor& key,
    const executorch::aten::Tensor& value,
    int32_t valid_length) {
  const auto validation =
      validate_fp32_layer_cache(layer, key, value, valid_length);
  if (validation != executorch::runtime::Error::Ok) {
    return validation;
  }
  return replace_layer_cache(layer, key, value, valid_length);
}
